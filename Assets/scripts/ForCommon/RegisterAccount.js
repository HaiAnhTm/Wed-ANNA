...
const notifyPasswordConfirm = document.getElementById(
  "notify_password_confirm"
);
...
inputUsername.addEventListener("blur", function (event) {
  if (inputEmpty(notifyUsername, inputUsername.value)) return;
  if (checkUsernameExist(inputUsername.value, notifyUsername)) return;
});

inputPassword.addEventListener("blur", function (event) {
  if (inputEmpty(notifyPassword, inputPassword.value)) return;
  if (inputPasswordShort(notifyPassword, inputPassword.value)) return;
});
...
inputPasswordConfirm.addEventListener("blur", function (event) {
  ...
  if (inputPasswordShort(notifyPasswordConfirm, inputPasswordConfirm.value))
    return;
  if (checkPasswordConfirmNotEquals()) return;
});

inputName.addEventListener("blur", function (event) {
  if (inputEmpty(notifyName, inputName.value)) return;
});
